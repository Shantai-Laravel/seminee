        ...
        <table class="table border-simple">
            ...
            <tbody>
                <tr v-for="(item, key) in items">
                    ...
                    <td>
                        <input type="checkbox" :id="'active'+key" v-model="actives[item.id]" @change="changeActive(item.id)">
                    </td>
                </tr>
            </tbody>
        </table>
...
export default {
    ...
    methods: {
        ...
        changeActive(id){
            bus.$emit('startLoading');
            axios.post('/back/translations-set-active', {id: id})
                .then(response => {
                    bus.$emit('endLoading');
                })
                .catch(e => {
                    console.log('error set active');
                })
        },
        changeStatus(){
