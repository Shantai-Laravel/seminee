<template>
    ...
    <div class="row" v-if="ready">
        <div class="col-12">
            <h3>{{ trans.vars.DetailsProductSet.recomanded }}</h3>
        </div>
        ...
        <div class="col-12" v-if="page <= last_page">
            <a href="#" class="butt" @click.prevent="showMore"><span>{{ trans.vars.DetailsProductSet.viewMore }}</span></a>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        load() {
            axios.post('/'+ this.$lang +'/recomanded?page=' + this.page)
                .then(response => {
